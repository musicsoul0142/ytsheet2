...
print <<"HTML";
          <tbody id="skill-row${num}">
            ...
            <tr><td colspan="8">
              <div>
                <b>取得元</b><select name="skill${num}Type" onchange="calcSkills();calcLvUpSkills();">@{[ option "skill${num}Type",'general|<一般>','race|<種族>','style|<流派>','geis|<誓約>','faith|<天恵>','add|<他スキル>',@experienced ]}</select>
